...
ThreadInfo::ThreadInfo(DWORD no)
{
	...
	{
		cout << "Unable to create the snapshot of currant thread pool" << endl;
		return;
	}
...
DLLInfo::DLLInfo(DWORD no)
{
	...
	{
		cout << "Error:Unable to create the snapshot of current thrad pool" << endl;
		return;
	}
...
BOOL DLLInfo::DependentDLLDisplay()
{
	...
	cout << "DEPENDENT DLL OF THE PROCESS" << endl;
	do
	{
...
{
	const char* month[] = { "JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DES" };
	...
	sprintf_s(FileName, "D://MarvellousLog %02d_%02d_%02d%s.txt", lt.wHour, lt.wMinute, lt.wDay, month[lt.wMonth - 1]);

	fp = fopen(FileName, "web");
	if (fp == NULL)
	{
	...
	{
		cout << "log file succssfully get created as:" << FileName << endl;
...
BOOL ProcessInfo::ReadLog(DWORD hr, DWORD min, DWORD date, DWORD month)
{
	...
	while ((ret = fread(&fobj, 1, sizeof(fobj), fp)) != 0)
	{
		...
		cout << "Parent process PID:" << fobj.thread_cnt << endl;
...
void DisplayHelp()
{
	...
	cout << "ps -d:Display all information about DDL" << endl;
		...
		else if (count == 2)
		{
			...
			else if (_stricmp(command[0], "kill") == 0)
			{
				...
				if (bret == FALSE)
					...
					cout << command[1] << "Terminated succesfull" << endl;
				delete ppobj;
